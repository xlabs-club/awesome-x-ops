...
先从需要控制的运维故障出发，而不是从最时髦的项目出发：需要 trace 和评估证据时看 **LLM 和 Agent 可观测性**，需要发布和运行时容量管理时看 **AI Serving and Inference Operations**，需要数据摄取和检索时看 **AI 基础设施** 或 **LLM 知识库**，需要策略、隔离和审计时看 **DevSecOps 与供应链安全**。然后只比较能够满足回滚、责任边界和数据保留要求的最小工具集合。
...
**选择建议：** 优先选择能将 trace 与 Prompt、模型版本、评估结果及成本或延迟信号关联起来的工具；对于 RAG，优先选择无需依赖脆弱标准答案集、同时衡量检索和回答质量的评估工作流；如果工具的主要职责不是可观测性，应放入专门的网关、安全或推理服务分类。对于 Agent 专用工具，还要确认工具调用、状态转换、失败过程和运维人员操作可被检查，而不是只有一个仪表盘。
...
- [NVIDIA SkillEvaluator](https://github.com/NVIDIA/SkillEvaluator)：基于 Apache-2.0 许可的框架，用于验证、执行安全扫描、去重并在线评估 AI Agent Skill，提供确定性质量门禁和可复现报告。
...
**RAG 发布门禁：** 分开记录数据摄取、检索和生成指标；固定解析器、Embedding 与重排模型版本；保留来源和分块标识以便回放；在全面发布前，用固定评测集对索引或 Prompt 变更执行金丝雀验证。如果检索质量回退，应先回滚索引或检索器，而不是直接换更大的模型补洞。
